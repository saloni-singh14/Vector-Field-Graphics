...
void VectorField()
{
    ///This function plots a vector field according to the given function F(x,y) using Bresenham's Line drawing algorithm
    ...
    int step = 40;
    /// The following loop runs for x: from xMin to xMax, and for y from yMin to yMax. It then uses Bresenham's algo to plot a vector with tail at each x,y in this range
    ...
    for (int i = xMin; i <= xMax; i += step)
    {
        for (int j = yMin; j <= yMax; j += step)
        {
            x = i;
            y = j;
            float distance = sqrt(x * x + y * y);//distance from center
            //F(x,y)=(y,-x)
            //Function corresponding to vector Field
            x_function = y;
            y_function = -x;
            /// x_function and y_function correspond to the function that determines the vector field
            setRGB(distance);
            ///We plot a line using Bresenham(). The line is drawn between (x,y) and (x+x_function,y+y_function) 
            //The line is drawn between(x, y) and (x + x_function, y + y_function)
            Bresenham(i, j, scale * x_function + i, j + scale * y_function);
        }
    }
    ///@see setRGB() 
    ///@see Bresenham()
...
void display() {
    ...
    R = 0, G = 1, B = 1;   
    VectorField();// Plots vector field
    glutSwapBuffers();
}
